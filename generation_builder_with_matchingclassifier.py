    ...
    def run_experiment(self):
        with tf.Session(config=tf.ConfigProto(allow_soft_placement=True)) as sess:
            ...
            if self.spherical_interpolation:
                ...
                # self.z_vectors_base = np.random.uniform(-1., 1., size=(1, self.z_dim))
                # for i in range(self.num_generations):
                #     self.z_vectors[i] = self.z_vectors_base[0]
                # inter = 16
                # for i in range(self.num_generations-4):
                #     if i > 0:
                #         # fix
                #         # self.z_vectors[i,inter*i:inter*(i+1)] = self.z_vectors_2[i,:inter]
                #         # random
                #         indexs = [i for i in range(self.z_dim)]
                #         s_index = random.sample(indexs,inter)
                #         self.z_vectors[i,s_index] = self.z_vectors_2[i,s_index]
            ...
            if self.continue_from_epoch >= 0:
                ...
                with tqdm.tqdm(total=self.total_gen_batches) as pbar_samp:
                    for i in range(self.total_gen_batches):
                        ...
                        root_recons = file_name.split('//')[0] +'_reconsimages'
                            ...
                            for j in range(1):
                                ...
                                if self.is_all_test_categories > 0:
                                    ...
                                    for j in range(self.batch_size):
                                        ...
                                        current_path_recons = file_name.split('//')[0] + '_recons' + '/{}/'.format(np.argmax(y_global_test_i[:, j]))
                                        ...
                                        if not os.path.exists(current_path_recons):
                                            os.makedirs(current_path_recons)
                                        ...
                                        for k in range(1 + self.data.support_number +  1 + 1 + each_samples_generation ):
                                            ...
                                            if k < 1 + self.data.support_number +  1 + 1 + 3:
                                                current_name_recons = current_path_recons + '{}.png'.format(k)
                                                scipy.misc.imsave(current_name_recons, current_iamge)
                                            ...
                                            if self.data.support_number -1 < k < self.data.support_number + 2:
                                                #### classifier ####
