...
def mean_fid(dataroot_real, dataroot_fake, dataset,image_size, channels,each_class_total_samples):
  ...
  for i in range(np.shape(real_images)[0]):
    # if i > 5:
    #   break
    print('11111',np.shape(real_images[i].repeat(30,axis=0)))
    FID = calculate_fid_given_paths(real_images[i].repeat(30,axis=0), fake_images[i].repeat(30,axis=0)) 
...
def mean_fid_tensorflow(dataroot_real, dataroot_fake, dataset,image_size, channels,each_class_total_samples):
    # real_images, fake_images  = get_real_fake_images(dataroot_real, dataroot_fake, dataset,image_size, channels,each_class_total_samples)
    # real_images_total = np.reshape(real_images, [np.shape(real_images)[0]*np.shape(real_images)[1], np.shape(real_images)[2],np.shape(real_images)[3],np.shape(real_images)[4]])
    # fake_images_total =  np.reshape(fake_images, [np.shape(real_images)[0]*np.shape(real_images)[1], np.shape(real_images)[2],np.shape(real_images)[3],np.shape(real_images)[4]])
    # print('maxmium',np.max(real_images_total),np.max(fake_images_total))
...
def calculate_fid_starganv2(dataroot_real, dataroot_fake, dataset,image_size, channels,each_class_total_samples):
  print('Calculating FID for all images...')
  fid_value = calculate_fid_given_paths_starganv2(
              paths=[dataroot_real, dataroot_fake],
              img_size=256,
              batch_size=32)
  print('total fid', fid_value)
  ...
  fid_mean = fid_value / len(path_list_fake)
  print('mean fid', fid_mean)
...
def frechet_inception_distance(dataroot_real, dataroot_fake, dataset,image_size, channels,each_class_total_samples, is_category):
    ...
    if is_category:
      for i in range(np.shape(real_images)[0]):
        # if i > 5:
        #   break
        # print('222222',np.shape(fake_images[i].repeat(30, axis=0)))
        # FID = get_fid(fcd, BATCH_SIZE, real_images[i].repeat(repeat_num, axis=0), fake_images[i].repeat(repeat_num, axis=0), inception_images, real_activation, fake_activation,
        #               activations) * 0.43
...
# inception_score(dataroot_real=args.dataroot_real,ataroot_fake=args.dataroot_fake, dataset=dataset,
#                            image_size=args.image_width, channels=args.image_channel,
#                            each_class_total_samples=args.augmented_support)
# frechet_inception_distance(dataroot_real=args.dataroot_real,ataroot_fake=args.dataroot_fake, dataset=dataset,
#                            image_size=args.image_width, channels=args.image_channel,
#                            each_class_total_samples=args.augmented_support)
