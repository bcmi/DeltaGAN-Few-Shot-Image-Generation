...
class DAGANDataset(object):
    ...
    def get_batch(self, dataset_name):
        ...
        ###nwaykshot
            x_input_batch_a = np.zeros(
                [self.batch_size, self.testing_classes, self.image_height, self.image_width,
                     self.image_channel])
            y_input_batch_a = np.zeros([self.batch_size, self.testing_classes, self.selected_classes])
            y_global_input_batch_a = np.zeros([self.batch_size, self.testing_classes, classes])

            x_input_batch_b = np.zeros(
                [self.batch_size, self.testing_classes * self.general_classification_samples, self.image_height, self.image_width,
                 self.image_channel])
            y_input_batch_b = np.zeros(
                [self.batch_size, self.testing_classes * self.general_classification_samples, self.selected_classes])
            y_global_input_batch_b = np.zeros([self.batch_size, self.testing_classes * self.general_classification_samples, self.testing_classes])

    #### for trained matchingGAN to generate fake images
            print('dataset name',dataset_name)
            print('total data',np.shape(self.datasets[dataset_name]))
            training_dataset = np.zeros([self.testing_classes,self.general_classification_samples,128, 128,3])
            for i in range(self.testing_classes):
                training_dataset[i,:,:,:,:] = self.datasets[dataset_name][i][:self.general_classification_samples]
            ...
            selected_classes = self.testing_classes
            support_number = self.general_classification_samples

            for i in range(self.batch_size):
                choose_classes = np.random.choice(len(training_dataset), size=selected_classes)
                for j in range(selected_classes):
                    ...
                    choose_samples_b = np.random.choice(training_dataset[choose_classes[j]].shape[0],
                                                        size=support_number, replace=False)

                    print(np.shape(testing_dataset),choose_classes[j],choose_samples_a[0])
                    x_input_batch_a[i, j, :, :, :] = self.resize(
                        testing_dataset[choose_classes[j],choose_samples_a[0],:,:,:])
                    # y_input_batch_a[i, j, j] = 1
                    y_global_input_batch_a[i, j, choose_classes[j]] = 1

                    for k in range(support_number):
                        x_input_batch_b[i, support_number * j + k, :, :, :] = self.resize(
                            training_dataset[choose_classes[j],
                                choose_samples_b[k]])
                        # y_input_batch_b[i, support_number * j + k, j] = 1
                        y_global_input_batch_b[i, support_number * j + k, choose_classes[j]] = 1

            for i in range(self.testing_classes):
                x_input_batch_a[:, i] = self.preprocess_data(x_input_batch_a[:, i])
            for j in range(self.testing_classes * self.general_classification_samples):
                x_input_batch_b[:, j] = self.preprocess_data(x_input_batch_b[:, j])

            print('hhh',np.shape(x_input_batch_b))
            return x_input_batch_a, x_input_batch_b, y_input_batch_a, y_input_batch_b, y_global_input_batch_a, y_global_input_batch_b
...
class FFHGAGEDAGANDataset(DAGANDataset):
    ...
    def load_dataset(self, gan_training_index):
        ...
        # self.x = self.x / 255
        # self.x = np.reshape(self.x, newshape=(2354, 100, 64, 64, 3))
        x_train, x_val, x_test = self.x[7:9], self.x[7:9], self.x[7:9]
        # x_train = x_train[:gan_training_index]
        return x_train, x_test, x_val
