...
if args.dataset == 'omniglot':
    ...
    origin_data = origin_dataset.SelectMOREanimalsDAGANDataset(batch_size=batch_size, last_training_class_index=900,
                                                   reverse_channels=True,
                                                   num_of_gpus=num_gpus, gen_batches=1000, support_number=support_num,
                                                   is_training=args.is_training,
                                                   general_classification_samples=args.general_classification_samples,
                                                   selected_classes=args.selected_classes, image_size=args.image_width)
    test_data = test_dataset.SelectMOREanimalsDAGANDataset(batch_size=batch_size, last_training_class_index=900,
                                               reverse_channels=True,
                                               num_of_gpus=num_gpus, gen_batches=1000, support_number=support_num,
                                               is_training=args.is_training,
                                               general_classification_samples=args.general_classification_samples,
...
def get_real_fake_images(fake_images_path):
    ...
    # print('hererere',np.shape(real_images))
...
#   print('funit total fid is', fid_value_total)
  # mFID = 0
  # i = 0
  # for i in range(np.shape(real_images)[0]):
  #   # if i > 5:
  #   #   break
  #   FID = calculate_fid_given_paths(real_images[i], fake_images[i])
  #   print('funit {}_category_fid'.format(i), FID)
  #   mFID+= FID
  #   i = i + 1

  # mFID = mFID / i
  # print("funit mean FID : ", mFID)
...
#### calculating the FIS from each category
