...
class ExperimentBuilder(object):
    def __init__(self, parser, data):
        ...
        self.input_x_i = tf.placeholder(tf.float32,
                                        [self.num_gpus, self.batch_size * self.data.selected_classes, image_height,
                                         image_width,
                                         image_channels], 'inputs-1')

        self.input_y = tf.placeholder(tf.float32, [self.num_gpus, self.batch_size * self.data.selected_classes,
                                                   self.data.selected_classes],
                                      'y_inputs-1')

        self.input_y_general = tf.placeholder(tf.float32, [self.num_gpus, self.batch_size * self.data.selected_classes,
        ...
        # self.Nway = self.data.selected_classes
        # self.kshot = args.general_classification_samples

        self.tensorboard_update_interval = int(self.total_test_batches / 10 / self.num_gpus)

        classifier = densenet_classifier(input_x_i=self.input_x_i, input_y=self.input_y_general,
        ...
        # print('{}_way_{}_shot'.format(self.data.selected_classes, self.data.general_classification_samples))
        print('{}_classification'.format(self.data.testing_classes))
    ...
    def run_experiment(self):
        ...
        with tf.Session(config=tf.ConfigProto(allow_soft_placement=True)) as sess:
            ...
            # checkpoint = './EVALUATION/AugmentedClassifier/animals/pretrained/saved_models/Pretrain_on_Source_Domain_9_0.5907641649246216.ckpt'
            ...
            with tqdm.tqdm(total=self.classification_total_epoch) as pbar_e:
                for e in range(self.classification_total_epoch):
                    ...
                    with tqdm.tqdm(total=self.total_test_batches) as pbar_samp:
                        for iter in range(int(self.total_test_batches)):
                            ...
                            y_test = np.reshape(y_test,
                                                [y_test.shape[0], y_test.shape[1] * y_test.shape[2], y_test.shape[3]])
                    ...
                    if self.training:
                        ...
                        model_name = 'Pretrain_on__Unseen_Categories'
                        ...
                        if total_test_accuracy_mean > lowest_d_val_accuracy:
                            ...
                            val_save_path = self.saver.save(sess, "{}/{}_{}_{}.ckpt".format(
                                self.saved_models_filepath,
                                model_name, e, total_test_accuracy_mean))
                            # val_save_path = self.saver.save(sess, "{}/{}_epoch{}.ckpt".format(
                            #     self.saved_models_filepath,
                            #     model_name, e))
                            # print("Saved current best val model at", val_save_path)
                    pbar_e.update(1)
