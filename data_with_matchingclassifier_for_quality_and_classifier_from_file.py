...
from data_preparation import generate_image_label_pairs_array
        ...
        if self.is_training > 0:
            ...
            for i in range(self.batch_size):
                ...
                for j in range(self.selected_classes):
                    ...
                    for k1 in range(testing_num):
                        # list = [i for i in range(len(training_dataset))]
                        # k = np.random.choice(list)
                        # x_input_batch_a[i, j*testing_num + k1, :, :, :] = self.resize(testing_dataset[k][0])
...
class animalsDAGANDataset(DAGANDataset):
    ...
    def load_dataset(self, gan_training_index):
        ...
        dataroot = '../Matching-DAGAN-1wayKshot/coarse-data/animals'
        dataset = 'animals'
        self.x = generate_image_label_pairs_array(dataroot=dataroot, dataset=dataset, image_size=128, channels=3, each_class_total_samples=600)
        print('data shape', np.shape(self.x))
        x_train, x_val, x_test = self.x[:120], self.x[100:120], self.x[:30]
        return x_train, x_test, x_val
