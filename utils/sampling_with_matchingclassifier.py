...
def sample_generator(num_generations, sess, same_images, dropout_rate, dropout_rate_value, data, batch_size, file_name,
    ...
    input_images, support_images, generated, reconstructed, refer_translation = data.reconstruct_original(input_images_list), data.reconstruct_original(
        support_list), data.reconstruct_original(generated_list), data.reconstruct_original(recons_images_list), data.reconstruct_original(refer_trans_list)
    images = np.zeros(
        shape=(batch_size, 1 + support_num + 1 + 1 + num_generations, support_images.shape[-3], support_images.shape[-2],
               support_images.shape[-1]))
    ...
    x_total_images = np.zeros([batch_size * (1 + support_num + 1+ 1 + num_generations), image_size, image_size, channel])
...
def sample_generator_store_into_file_fid_generation_real(num_generations, sess, same_images, dropout_rate, dropout_rate_value, data,
                                     batch_size, file_name,
                                     conditional_inputs, input_a,
                                     support_input, input_b,
                                     y_input_i, input_y_i,
    ...
    support_num = support_images.shape[-4]
    input_images_list = np.zeros(shape=(batch_size, each_samples_generation, input_images.shape[-3], input_images.shape[-2],
                                        input_images.shape[-1]))
    generated_list = np.zeros(shape=(batch_size, each_samples_generation, generated.shape[-3], generated.shape[-2],
                                     generated.shape[-1]))

    support_list = np.zeros(shape=(
        batch_size, each_samples_generation, support_images.shape[-4], support_images.shape[-3], support_images.shape[-2],
        support_images.shape[-1]))

    recons_images_list = np.zeros(shape=(batch_size, each_samples_generation, input_images.shape[-3], input_images.shape[-2],
                                         input_images.shape[-1]))

    refer_trans_list = np.zeros(shape=(batch_size, each_samples_generation, input_images.shape[-3], input_images.shape[-2],
                                       input_images.shape[-1]))
    ...
    images = np.zeros(
        shape=(
        batch_size, 1 + support_num + 1 + 1 + each_samples_generation, support_images.shape[-3], support_images.shape[-2],
        support_images.shape[-1]))
    ...
    y_few_shot_total_image = np.zeros([batch_size * (1 + support_num + 1 + 1 + each_samples_generation), selected_classes])
    ...
    for j in range(batch_size):
        ...
        for k in range(1 + support_number + 1 + 1 + num_generations):
            ...
            if k>0 and k < support_num + 1:
                ...
                if (current_iamge.shape[-2] < 128):
                    current_iamge = cv2.resize(current_iamge, (128, 128), interpolation=cv2.INTER_LINEAR)
                current_name = current_path + file_name.split('//')[-1].split('.png')[0] + '_conditional_{}.png'.format(k)
                scipy.misc.imsave(current_name, current_iamge)
            elif k==support_num + 1:
                current_iamge = images[image_size * j:image_size * (j + 1), image_size * (k):image_size * (k + 1)]
                if (current_iamge.shape[-2] < 128):
                    current_iamge = cv2.resize(current_iamge, (128, 128), interpolation=cv2.INTER_LINEAR)
                current_name = current_path + file_name.split('//')[-1].split('.png')[0] + '_cyclerecons_{}.png'.format(k)
            elif k == support_num + 2:
                current_iamge = images[image_size * j:image_size * (j + 1), image_size * (k):image_size * (k + 1)]
                if (current_iamge.shape[-2] < 128):
                    current_iamge = cv2.resize(current_iamge, (128, 128), interpolation=cv2.INTER_LINEAR)
                ...
                if (find_classes(current_path) < num_generations):
                    ...
                    current_name = current_path + file_name.split('//')[-1].split('.png')[0] + '_sample_{}.png'.format(k)
                    scipy.misc.imsave(current_name, current_iamge)
