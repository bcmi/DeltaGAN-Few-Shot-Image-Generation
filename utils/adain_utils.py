...
def pre_resblock(x_init, channels, use_bias=False, sn=False, scope='resblock'):
    with tf.variable_scope(scope):
...
def pre_adaptive_resblock(x_init, channels, gamma1, beta1, gamma2, beta2, use_bias=False, sn=False, scope='adaptive_resblock') :
    with tf.variable_scope(scope):
        _, _, _, init_channel = x_init.get_shape().as_list()
        with tf.variable_scope('res1'):
            x = lrelu(x_init)
            x = adaptive_instance_norm(x, gamma1, beta1)
            x = conv(x, channels, kernel=1, stride=1, use_bias=use_bias, bias_init=tf.constant_initializer(1.0), sn=sn)
            # norm_value = tf.norm(x)
            # mean, variance = tf.nn.moments(x, axes=[0, 1, 2, 3])
            # tf.summary.scalar('pre_adaptive_resblock_norm_1', norm_value)
            # tf.summary.scalar('pre_adaptive_resblock_mean_1', mean)
            # tf.summary.scalar('pre_adaptive_resblock_var_1', variance)
        ...
        with tf.variable_scope('res2'):
            ...
            # norm_value = tf.norm(x)
            # mean, variance = tf.nn.moments(x, axes=[0, 1, 2, 3])
            # tf.summary.scalar('pre_adaptive_resblock_norm_2', norm_value)
            # tf.summary.scalar('pre_adaptive_resblock_mean_2', mean)
            # tf.summary.scalar('pre_adaptive_resblock_var_2', variance)
