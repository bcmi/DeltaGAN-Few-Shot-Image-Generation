...
class generate_image_label_pairs():
    def __init__(self, dataroot, dataset):
        ...
        self.dataset = dataset
        self.data_loader = FIGR_Omniglot(self.dataroot, dataset,transform=None)
        ...
        for (img, label) in self.dataloader:
            if label in temp:
                ...
                temp[label] = [img]
        self.dataloader_training = []  # Free memory
        self.dataloader_testing = []

        #### training_class_num
        i = 0
        for classes in temp.keys():
            if i < training_class_num:
                self.dataloader_training.append(classes,temp[list(temp.keys())[classes]])
            else:
                self.dataloader_testing.append(classes,temp[list(temp.keys())[classes]])
            i=i+1

        training_classes = len(self.dataloader_training)    
        testing_classes = len(self.dataloader_testing)
        print('training classes', training_classes)
        print('testing_classes',testing_classes)
        return self.dataloader_training, self.dataloader_testing
...
class DAGANDataset(object):
    def __init__(self, batch_size, last_training_class_index, reverse_channels, num_of_gpus, gen_batches,
        ...
        self.image_channel = self.image_channel
        ## classes
        self.training_classes = len(self.dataloader_training)
        self.testing_classes = len(self.dataloader_testing)
        self.val_classes =  len(self.dataloader_testing)
        ## classes * samples
    ...
    def get_batch(self, dataset_name):
        ...
        if self.is_training > 0:
            ...
            for i in range(self.batch_size):
                ...
                # choose_classes = [(i*self.selected_classes+j) for j in range(self.selected_classes)]
                for j in range(self.selected_classes):
                    #print('oairs',len(self.datasets[dataset_name][0]))
                    choose_pairs = np.random.choice(len(self.datasets[dataset_name][0]), size=self.selected_classes) 
                    #choose_samples = np.random.choice(pairs_index, size=1, replace=False)
                    
                    x_input_batch_a[i, j, :, :, :] = self.resize(self.datasets[dataset_name][choose_classes[j]][choose_pairs[j]][0])
                    y_input_batch_a[i, j, j] = 1
                    y_global_input_batch_a[i, j, choose_classes[j]] = 1

                    for k in range(self.support_number):
                        x_input_batch_b[i, self.support_number * j + k, :, :, :] = \
                            self.resize(self.datasets[dataset_name][choose_classes[j]][choose_pairs[j]][k])
                        y_input_batch_b[i, self.support_number * j + k, j] = 1
                        y_global_input_batch_b[i, self.support_number * j + k, choose_classes[j]] = 1
...
class animalsDAGANDataset(DAGANDataset):
    ...
    def load_dataset(self, gan_training_index):
        self.dataloader_training, self.dataloader_testing = generate_image_label_pairs(store_file, 'animals')
        self.x = np.load("../Matching-DAGAN-1wayKshot/datasets/animals_3_100_3_100_data.npy")
        return self.dataloader_training, self.dataloader_testing


# (149, 100, 84, 84, 3)
class SelectanimalsDAGANDataset(DAGANDataset):
    def __init__(self, batch_size, last_training_class_index, reverse_channels, num_of_gpus, gen_batches,
                 support_number, is_training, general_classification_samples, selected_classes, image_size):
        super(SelectanimalsDAGANDataset, self).__init__(batch_size, last_training_class_index, reverse_channels, num_of_gpus,
                                                  gen_batches, support_number, is_training,
                                                  general_classification_samples, selected_classes, image_size)

    def load_dataset(self, gan_training_index):
        self.x = np.load("../Matching-DAGAN-1wayKshot/datasets/AnimalFaceEasyPairs.npy")
...
class GenerateanimalsDAGANDataset(DAGANDataset):
    def __init__(self, batch_size, last_training_class_index, reverse_channels, num_of_gpus, gen_batches,
                 support_number, is_training, general_classification_samples, selected_classes, image_size):
        super(GenerateanimalsDAGANDataset, self).__init__(batch_size, last_training_class_index, reverse_channels, num_of_gpus,
                                                  gen_batches, support_number, is_training,
                                                  general_classification_samples, selected_classes, image_size)

    def load_dataset(self, gan_training_index):
        self.x = np.load("../Matching-DAGAN-1wayKshot/datasets/AnimalFaceEasyPairsII.npy")
        self.test_x = np.expand_dims(np.load("../Matching-DAGAN-1wayKshot/datasets/AnimalFaceTest.npy"),axis=2)
        print('data shape', np.shape(self.x))
        # print('here',np.min(self.x[:100],axis=(0,1,2,3)),np.mean(self.x[:100],axis=(0,1,2,3)),np.max(self.x[:100],axis=(0,1,2,3)),np.std(self.x[:100],axis=(0,1,2,3)))
        self.x = self.x / np.max(self.x)
