...
class DAGANDataset(object):
    def __init__(self, batch_size, last_training_class_index, reverse_channels, num_of_gpus, gen_batches,
        ...
        # print('here',np.shape(self.x_train))
    ...
    def get_batch(self, dataset_name):
        ...
        # few_shot_classes = np.random.choice(int(self.datasets[dataset_name].shape[0]), size=self.selected_classes, replace=False)
        training_dataset = [ self.datasets[dataset_name][i][testing_num:] for i in range(classes)]
        testing_dataset =  [ self.datasets[dataset_name][i][:testing_num] for i in range(classes)]
        self.training_data_size = np.sum([len(training_dataset[i]) for i in range(len(training_dataset))])
        self.testing_data_size = np.sum([len(testing_dataset[i]) for i in range(len(testing_dataset))])
        # print('training images size', self.training_data_size)
        # print('testing images size', self.testing_data_size)

        total_classes = np.array([i for i in range(classes)])
        for i in range(self.batch_size):
            # choose_classes = np.array( [i for i in range(classes)] )

            choose_classes = np.random.choice(total_classes,size=self.selected_classes, replace=False)
            ...
            for j in range(self.selected_classes):
                ...
                x_input_batch_a[i, j, :, :, :] = self.resize(testing_dataset[choose_classes[j]][choose_samples_a[0]])
...
class VGGFaceDAGANDataset(DAGANDataset):
    ...
    def load_dataset(self, gan_training_index):
        ...
        x_train, x_val, x_test = self.x[1802:1898], self.x[1802:1898], self.x[1802:1898]
        # self.x = self.x * 255
...
class animalsDAGANDataset(DAGANDataset):
    ...
    def load_dataset(self, gan_training_index):
        # ##### pretrained_model
        self.x = np.load("../Matching-DAGAN-1wayKshot/datasets/animals_c117484_s128_data.npy")[120:]
        self.x = self.x / 255
        print('real test images',np.shape(self.x))
        # x_train, x_val, x_test = self.x[:120], self.x[:120], self.x[:120]
        # print('fake data shape', np.shape(x_test))
        ...
        ##### few-shot training
        # self.x = np.load("./EVALUATION/Augmentednpy/animals/MSDcombinedDx1x25shot/visual_outputs_forclassifier/Augmented_515.npy")
        # self.x = np.load("./EVALUATION/Augmentednpy/animals/MSDcombinedDx1x21shot/visual_outputs_forclassifier/Augmented_111.npy")
        # self.x = np.load("./EVALUATION/Augmentednpy/animals/MSDcombinedDx1x21shot/visual_outputs_forclassifier/Augmented_111.npy")
        # self.x = self.x / 255
        # print('fake data shape', np.shape(self.x))
        x_train, x_val, x_test = self.x[:], self.x[:], self.x[:]
        ##### few-shot training
        return x_train, x_test, x_val
...
class flowersDAGANDataset(DAGANDataset):
    ...
    def load_dataset(self, gan_training_index):
        ...
        self.x = self.x / 255
        x_train, x_val, x_test = self.x[85:], self.x[85:], self.x[85:]
...
class SelectMOREanimalsDAGANDataset(DAGANDataset):
    ...
    def load_dataset(self, gan_training_index):
        ...
        # self.x = np.load("./EVALUATION/Augmentednpy/animals/MSDMMD/dataset_forclassifier/Augmented_250.npy")
        # self.x = np.load("./EVALUATION/Augmentednpy/selectanimals/MSDMMD/dataset_forclassifier/Augmented_250.npy")
        # self.x = np.load("./EVALUATION/Augmentednpy/selectanimals/MSDMMD/dataset_forclassifier/Augmented_250.npy")
        # self.x = np.load("./EVALUATION/Augmentednpy/selectanimals/MSDAdversarial/dataset_forclassifier/Augmented_250.npy")
        self.x = np.load("./EVALUATION/Augmentednpy/SelectMOREanimals/MSDcombinedDx1x21/dataset_forclassifier/Augmented_380.npy")
        print('data shape', np.shape(self.x))
        self.x = self.x / 255
        x_train, x_val, x_test = self.x[:], self.x[:], self.x[:]
        return x_train, x_test, x_val
...
class NAbirdsDAGANDataset(DAGANDataset):
    ...
    def load_dataset(self, gan_training_index):
        ...
        x_train, x_val, x_test = self.x[444:], self.x[444:], self.x[444:]
        # x_train = x_train[:gan_training_index]
